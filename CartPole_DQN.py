...
def create_features(state: np.ndarray) -> np.ndarray:
    ...
    features = np.array([
        ...
        # TODO: Continue with polynomial terms
    ])
...
class LinearQNetwork:
    ...
    def update(
        ...
        self.weights[action] = self.weights[action] + self.learning_rate*td_error*features
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            if done:
                ...
                target = reward  + gamma*max(next_q_values)# REPLACE: add discounted max future Q-value
...
class ReplayBuffer:
    ...
    def sample(
        ...
        import random 
        batch = random.sample(self.buffer, batch_size) # REPLACE THIS LINE

        # Unpack batch into separate arrays
        states = [i[0] for i in batch]  # REPLACE
        actions = [i[1] for i in batch] # REPLACE
        rewards = [i[2] for i in batch]  # REPLACE
        next_states = [i[3] for i in batch]  # REPLACE
        dones = [i[4] for i in batch]  # REPLACE
...
class QNetwork(nn.Module):
    ...
    def forward(self, state: torch.Tensor) -> torch.Tensor:
        ...
        # ============================================
        return self.network(state)  # REPLACE THIS LINE
...
def train_dqn(
    ...
    q_network = QNetwork(state_dim, num_actions)# REPLACE THIS LINE
    target_network = QNetwork(state_dim, num_actions)
    target_network.load_state_dict(q_network.state_dict())  # REPLACE THIS LINE

    # Copy weights to target network
    # target_network.load_state_dict(q_network.state_dict())

    # Create optimizer
    optimizer = torch.optim.Adam(q_network.parameters(), lr=learning_rate)  # REPLACE THIS LINE
    # Hint: Use torch.optim.Adam(q_network.parameters(), lr=learning_rate)

    # Create replay buffer
    replay_buffer = ReplayBuffer(buffer_capacity) # REPLACE THIS LINE
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            if np.random.random() < epsilon:
                ...
                with torch.no_grad():
                    q_values = q_network(state_tensor) # REPLACE: get Q-values from q_network

                # Select greedy action
                action = int(torch.argmax(q_values, dim=1).item())   # REPLACE: argmax over Q-values
            ...
            if len(replay_buffer) >= batch_size:
                ...
                current_q_values = q_network.forward(states) # REPLACE: Q-network predictions

                # Get Q-values for actions that were taken
                current_q = current_q_values.gather(1, actions.unsqueeze(1)).squeeze(1)  # REPLACE: gather Q-values for taken actions
                # Hint: Use current_q_values.gather(1, actions.unsqueeze(1)).squeeze(1)

                # Compute target Q-values using target network
                with torch.no_grad():
                    next_q_values = target_network.forward(next_states)  # REPLACE: target network predictions
                    max_next_q = next_q_values.max(1)[0]  # REPLACE: max over actions
                    # Hint: next_q_values.max(1)[0]

                    # TD target: r + gamma * max_a' Q(s', a') (or just r if done)
                    target_q = rewards + gamma * max_next_q * (1 - dones) # REPLACE: compute TD target
                    # Hint: rewards + gamma * max_next_q * (1 - dones)

                # Compute loss
                loss = nn.functional.mse_loss(current_q, target_q) # REPLACE: MSE loss between current_q and target_q
                # Hint: nn.functional.mse_loss(current_q, target_q)
            ...
            if step_count % target_update_freq == 0:
                # TODO: Copy weights from q_network to target_network
                target_network.load_state_dict(q_network.state_dict())
